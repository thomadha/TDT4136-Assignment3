...
def alpha_beta_search(game: Game, state: State, player) -> Action | None:
    #player = game.to_move(state)
    value, move = max_valueAB(game, state, -math.inf, math.inf, player)  # do a direct call instead.
    return move

def max_valueAB(game: Game, state: State, a, b, player):
    if game.is_terminal(state):
        return game.utility(state, player), None  # Pass the correct player
    v = -math.inf  # Start with negative infinity
    for move in game.actions(state):
        value, _ = min_valueAB(game, game.result(state, move), a, b, player)
        if value > v:
            v = value
            best_move = move
            a = max(a,v)
...
def min_valueAB(game: Game, state: State, a, b, player):
    if game.is_terminal(state):
        return game.utility(state, player), None  # Pass the correct player
    v = math.inf  # Start with infinity
    for move in game.actions(state):
        value, _ = max_valueAB(game, game.result(state, move), a, b, player)
        if value < v:
            v = value
            best_move = move
...
while not game.is_terminal(state):
    player =  game.to_move(state)
    action =  alpha_beta_search(game, state, player) # The player whose turn it is
                                             # is the MAX player
